...
def remove_low_quality_proxies():
    ...
    deleted_count, _ = Proxy.objects.filter(
        is_active=False,
        times_checked__gt=1,
        times_check_succeeded__lt=Coalesce(F("times_checked") * LOW_QUALITY_THRESHOLD, 0, output_field=FloatField()),
    ).delete()
...
def update_status():
    ...
    if req.status_code == 204:
        ...
        TaskLog.objects.create(name="update_status", start_time=start_time, finish_time=now())
    else:
        print("The Shadowmere host is having connection issues. Skipping test cycle.")
